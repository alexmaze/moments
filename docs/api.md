...
**描述**: 删除指定空间。仅空间创建者（`owner`）可以删除。删除空间会软删除空间记录，并清理空间成员、宝宝成长记录和空间封面引用；空间内已有动态会保留为普通动态，其 `spaceId` 会被置空，动态附件、录音、点赞、评论、标签和提及不受影响。空间封面如不再被引用，会进入媒体 orphaned 清理流程。
...
- `403 Forbidden` — 当前用户不是空间创建者
- `404 Not Found` — 空间不存在或已删除
